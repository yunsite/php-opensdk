 ...
 * 2、include_once 'OpenSDK/163/Weibo.php';
 * 3、OpenSDK_163_Weibo::init($appkey,$appsecret);
 * 4、OpenSDK_163_Weibo::getRequestToken($callback); 获得request token
 * 5、OpenSDK_163_Weibo::getAuthorizeURL($token); 获得跳转授权URL
 * 6、OpenSDK_163_Weibo::getAccessToken($oauth_verifier) 获得access token
 * 7、OpenSDK_163_Weibo::call();调用API接口
 *
 * 建议：
...
class OpenSDK_163_Weibo extends OpenSDK_OAuth_Interface
{
    ...
    private static $authorizeURL = 'http://api.t.163.com/oauth/authenticate';

    private static $requestTokenURL = 'http://api.t.163.com/oauth/request_token';
     ...
     * @return string
     */
    public static function getAuthorizeURL($token , $callback=null)
    {
        if(is_array($token))
        {
    ...
    /**
     * 获得Access Token
     * @param string $oauth_verifier 网易微博不返回这个参数获取accessToken时也不需要
     * @return array
     */
